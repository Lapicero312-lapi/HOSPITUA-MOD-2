...
- **Guest**: Representa al huésped extranjero reportado. Atributos: `fullName`, `documentNumber`,
...
- **MigratoryValidation**: Representa la validación migratoria local realizada por el Módulo 2
  durante el Check-In mediante "Procesar datos de huésped extranjero". Atributos: `guestRef`,
  `submittedData` (pasaporte, nacionalidad, tipo de visa, fechas de estadía), `result` (`PASSED` |
  `FAILED`; solo se exportan los `PASSED`), `missingFields`, y `sireExportStatus` con dos estados
  oficiales: `PENDING` (validado localmente, aún no exportado) y `EXPORTED` (ya incluido en una
  descarga).
