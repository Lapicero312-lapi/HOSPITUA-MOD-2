...
2. **Scenario**: Rechazo de notificación para reservas sin ingreso (Error)
   ...
   - **Then** el sistema prohíbe el cambio de estado, responde **HTTP 400 (Bad Request)** informando
     que la reserva aún no registra un ingreso, y registra una incidencia de conciliación con el
     Módulo 1
...
- **FR-002**: El sistema debe validar que la `Reservation` esté en `IN_PROGRESS` antes de procesar
  la salida; si ya está en `COMPLETED` debe responder 200 sin efectos (idempotencia), y en cualquier
  otro estado debe responder **HTTP 400** y registrar una incidencia de conciliación.
...
- **Reservation**: Reserva que transiciona de `IN_PROGRESS` a `COMPLETED`. Atributos:
  ...
  `CANCELLED`, `NO_SHOW`).
- **ReconciliationIncident**: Discrepancia con el Módulo 1 que una persona debe resolver, registrada
...
- **SC-002**: El 100% de los intentos sobre reservas sin ingreso responden **HTTP 400** con
  incidencia registrada, y los duplicados sobre reservas ya finalizadas responden 200 sin efectos,
  sin generar errores de sistema.
